...
func (a *API) TraceBlockByHash(ctx context.Context, hash common.Hash, config *rpctypes.TraceConfig) ([]*rpctypes.TxTraceResult, error) {
	...
	height, err := backend.BlockNumberFromTendermintByHash(hash)
	if err != nil {
		a.logger.Debug("get block failed", "hash", hash.Hex(), "error", err.Error())
		return nil, errors.New("block not found")
	}
	return backend.TraceBlock(rpctypes.NewBlockNumber(height), config, nil)
}
...
func (a *API) StartCPUProfile(file string) error {
	...
	switch {
	case isCPUProfileConfigurationActivated():
