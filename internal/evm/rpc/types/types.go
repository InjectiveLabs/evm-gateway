...
// stateDB defines the state mutation API needed by overrides.
...
func (diff *StateOverride) Apply(db stateDB) error {
	if diff == nil {
		return nil
	}
	for addr, account := range *diff {
		...
		if account.Balance != nil {
			...
			if overflow {
				return fmt.Errorf("account %s balance overflow", addr.Hex())
			}
