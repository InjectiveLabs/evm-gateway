...
func (b *Backend) Resend(args rpctypes.TransactionArgs, gasPrice *hexutil.Big, gasLimit *hexutil.Uint64) (common.Hash, error) {
	...
	for _, tx := range pending {
		// FIXME does Resend api possible at all?  https://github.com/InjectiveLabs/injective-core/injective-chain/issues/905
...
func (b *Backend) SendRawTransaction(data hexutil.Bytes) (common.Hash, error) {
	...
	syncCtx := b.broadcastClientCtx.WithBroadcastMode(flags.BroadcastSync)
	rsp, err := syncCtx.BroadcastTx(txBytes)
	if rsp != nil && rsp.Code != 0 {
		err = errorsmod.ABCIError(rsp.Codespace, rsp.Code, rsp.RawLog)
	}
	if err != nil {
		err = unwrapBroadcastTxError(err)
...
func unwrapBroadcastTxError(err error) error {
	...
	switch {
	case rpcErr.Data != "":
		return fmt.Errorf("%s", rpcErr.Data)
	case rpcErr.Message != "":
...
func (b *Backend) SetTxDefaults(args rpctypes.TransactionArgs) (rpctypes.TransactionArgs, error) {
	...
	if args.Gas == nil {
		...
		callArgs := rpctypes.TransactionArgs{
			From:                 args.From,
			To:                   args.To,
			Gas:                  args.Gas,
